...
#### 6. Path-Parameter Header / Log Injection

**Location:** `src/index.ts` — routes shaped like
`/api/v1/pairs/:source/:destination`, `/api/v1/pairs/:source/:destination/*`,
and the equivalent body / query-param asset-code fields on
`POST /api/v1/pairs`, `POST /api/v1/pairs/bulk`, and the quote endpoints.

**Risk:** An attacker submitting CR (`%0D`), LF (`%0A`), CRLF (`%0D%0A`),
NUL (`%00`), TAB, or other control characters inside `:source`,
`:destination`, or `source_asset` / `dest_asset` could:
1. **Split HTTP response headers** by injecting CRLF followed by a forged
   header name/value when the code interpolates the raw param into a
   response header (e.g. ETag, `X-Injected`).
2. **Forge structured log lines** when CR/LF bytes leak into `pino`'s JSON
   fields, allowing a second, attacker-controlled, log entry to be
   injected in the middle of a single HTTP request's log output.
3. **Collide with the internal `__health` probe namespace** if the
   reserved `__HEALTH_PROBE_KEY` sentinel (used by the deep-health
   storage check) could ever be reached via a user-supplied asset code.

**Mitigation (implemented):** Every entry point that accepts asset codes —
path params, JSON body fields, URL query params, and bulk array items —
passes the value through `normalizeAsset` (indirectly via
`normalizePairParams` for path-param routes) **before** the value is ever
interpolated into a response body, event payload, ETag, header, or pair
registry key.

`normalizeAsset` enforces:
- Trim + uppercase first.
- Strict character class: `^[A-Z0-9]{1,12}$`. Any ASCII control byte
  (0x00–0x1F, 0x7F), punctuation, slash, dot, space, or byte outside
  `[A-Z0-9]` is rejected with `400 invalid_request` *before* the value
  touches any other layer.
- Reserved-prefix guard: codes beginning with `__HEALTH` are rejected
  explicitly so the internal `\x00__health_probe__` sentinel is never
  reachable from user input (even if a future encoding bug ever
  canonicalized NUL prefixes away).

All pair events recorded via `recordEvent` — `pair.registered`,
`pair.refreshed`, `pair.unregistered`, `pair.meta.reset`, `pair.enabled`,
`pair.disabled` — store only the post-`normalizeAsset` canonical value,
never the raw caller input. The `pairRegistry` Set keys are similarly
always built from canonicalized codes, so a path-param injection that
returns 400 never mutates stored state.

Coverage for this control lives in `src/__tests__/pathParamInjection.test.ts`
(CR/LF/NUL cases across all 10 path-param routes, `__health` prefix
rejection in path + body + bulk + quote params, header-splitting checks,
event-payload sanitization assertions, and registry-integrity tests).
